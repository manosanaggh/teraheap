...
void Universe::check_alignment(uintx size, uintx alignment, const char *name) {
  if (size < alignment || size % alignment != 0) {
    vm_exit_during_initialization(
        err_msg("Size of %s (" UINTX_FORMAT
                " bytes) must be aligned to " UINTX_FORMAT " bytes",
                name, size, alignment));
  }
}
...
void Universe::genesis(TRAPS) {
...

  {
...

    {
      ...
      // determine base vtable size; without that we cannot create the array
      // klasses
      compute_base_vtable_size();

      if (!UseSharedSpaces) {
        _boolArrayKlassObj =
            TypeArrayKlass::create_klass(T_BOOLEAN, sizeof(jboolean), CHECK);
        _charArrayKlassObj =
            TypeArrayKlass::create_klass(T_CHAR, sizeof(jchar), CHECK);
        _singleArrayKlassObj =
            TypeArrayKlass::create_klass(T_FLOAT, sizeof(jfloat), CHECK);
        _doubleArrayKlassObj =
            TypeArrayKlass::create_klass(T_DOUBLE, sizeof(jdouble), CHECK);
        _byteArrayKlassObj =
            TypeArrayKlass::create_klass(T_BYTE, sizeof(jbyte), CHECK);
        _shortArrayKlassObj =
            TypeArrayKlass::create_klass(T_SHORT, sizeof(jshort), CHECK);
        _intArrayKlassObj =
            TypeArrayKlass::create_klass(T_INT, sizeof(jint), CHECK);
        _longArrayKlassObj =
            TypeArrayKlass::create_klass(T_LONG, sizeof(jlong), CHECK);
        ...
        ClassLoaderData *null_cld =
            ClassLoaderData::the_null_class_loader_data();

        _the_array_interfaces_array =
            MetadataFactory::new_array<Klass *>(null_cld, 2, NULL, CHECK);
        _the_empty_int_array =
            MetadataFactory::new_array<int>(null_cld, 0, CHECK);
        _the_empty_short_array =
            MetadataFactory::new_array<u2>(null_cld, 0, CHECK);
        _the_empty_method_array =
            MetadataFactory::new_array<Method *>(null_cld, 0, CHECK);
        _the_empty_klass_array =
            MetadataFactory::new_array<Klass *>(null_cld, 0, CHECK);
      }
    }
    ...
    if (UseSharedSpaces) {
      ...
      _the_array_interfaces_array->at_put(0,
                                          SystemDictionary::Cloneable_klass());
      _the_array_interfaces_array->at_put(
          1, SystemDictionary::Serializable_klass());
    }
  ...
  if (JDK_Version::is_partially_initialized()) {
    ...
    if (k == NULL) {
      k = SystemDictionary::resolve_or_null(vmSymbols::java_lang_CharSequence(),
                                            THREAD);
      CLEAR_PENDING_EXCEPTION; // ignore exceptions
      if (k == NULL) {
        k = SystemDictionary::resolve_or_null(vmSymbols::java_lang_Shutdown(),
                                              THREAD);
        CLEAR_PENDING_EXCEPTION; // ignore exceptions
        if (k == NULL) {
          jdk_version = 2;
  ...
  if (FullGCALot) {
    ...
    objArrayOop naked_array =
        oopFactory::new_objArray(SystemDictionary::Object_klass(), size, CHECK);
    objArrayHandle dummy_array(THREAD, naked_array);
    int i = 0;
    while (i < size) {
      // Allocate dummy in old generation
      oop dummy = InstanceKlass::cast(SystemDictionary::Object_klass())
...
void Universe::fixup_mirrors(TRAPS) {
  // Bootstrap problem: all classes gets a mirror (java.lang.Class instance)
  // assigned eagerly, but we cannot do that for classes created before
  // java.lang.Class is loaded. Here we simply walk over permanent objects
  // created so far (mostly classes) and fixup their mirrors. Note that the
  // number of objects allocated at this point is very small.
  assert(SystemDictionary::Class_klass_loaded(),
         "java.lang.Class should be loaded");
...
void Universe::reinitialize_vtable_of(KlassHandle k_h, TRAPS) {
  ...
  if (ko->oop_is_instance()) {
    ...
    for (KlassHandle s_h(THREAD, ik->subklass()); s_h() != NULL;
...
bool Universe::should_fill_in_stack_trace(Handle throwable) {
  // never attempt to fill in the stack trace of preallocated errors that do not
  // have backtrace. These errors are kept alive forever and may be "re-used"
  // when all preallocated errors with backtrace have been consumed. Also need
  // to avoid a potential loop which could happen if an out of memory occurs
  // when attempting to allocate the backtrace.
...
oop Universe::gen_out_of_memory_error(oop default_err) {
  ...
  // - if there is a preallocated error with backtrace available then return it
  // wth
  ...
  if (_preallocated_out_of_memory_error_avail_count > 0) {
    ...
    assert(next < (int)PreallocatedOutOfMemoryErrorCount,
           "avail count is corrupt");
  } else {
    next = -1;
  }
  if (next < 0) {
...
char *Universe::preferred_heap_base(size_t heap_size, size_t alignment,
                                    NARROW_OOP_MODE mode) {
  assert(is_size_aligned((size_t)OopEncodingHeapMax, alignment), "Must be");
  assert(is_size_aligned((size_t)UnscaledOopHeapMax, alignment), "Must be");
  assert(is_size_aligned(heap_size, alignment), "Must be");

  uintx heap_base_min_address_aligned =
      align_size_up(HeapBaseMinAddress, alignment);

  size_t base = 0;
#ifdef _LP64
  if (UseCompressedOops) {
    assert(mode == UnscaledNarrowOop || mode == ZeroBasedNarrowOop ||
               mode == HeapBasedNarrowOop,
           "mode is invalid");
    ...
    if (!FLAG_IS_DEFAULT(HeapBaseMinAddress) && (mode == UnscaledNarrowOop)) {
    ...
    } else if ((total_size <= OopEncodingHeapMax) &&
               (mode != HeapBasedNarrowOop)) {
        ...
        if (mode == UnscaledNarrowOop ||
          ...
          if (UseCompressedClassPointers && !UseSharedSpaces &&
            ...
            assert(is_size_aligned((size_t)OopEncodingHeapMax - class_space,
                                   alignment),
                   "difference must be aligned too");
      ...
      // UnscaledNarrowOop encoding didn't work, and no base was found for
      // ZeroBasedOops or HeapBasedNarrowOop encoding was requested.  So, can't
      // reserve below 32Gb.
      Universe::set_narrow_oop_shift(LogMinObjAlignmentInBytes);
    }
...
jint Universe::initialize_heap() {
  ...
  if (UseCompressedOops) {
    ...
    if (((uint64_t)Universe::heap()->reserved_region().end() >
      ...
      // are decoded so that NULL is preserved, so this page will not be
      // accessed.
      Universe::set_narrow_oop_use_implicit_null_checks(false);
#endif
    } else {
...
const char *
Universe::narrow_oop_mode_to_string(Universe::NARROW_OOP_MODE mode) {
  switch (mode) {
...
bool universe_post_init() {
  ...
  k = SystemDictionary::resolve_or_fail(vmSymbols::java_lang_OutOfMemoryError(),
                                        true, CHECK_false);
  ...
  // (this is currently used for a cheap & dirty solution in compiler exception
  // handling)
  ...
  if (!DumpSharedSpaces) {
    // These are the only Java fields that are currently set during shared space
    // dumping. We prefer to not handle this generally, so we always
    // reinitialize these detail messages.
    Handle msg =
        java_lang_String::create_from_str("Java heap space", CHECK_false);
    java_lang_Throwable::set_message(Universe::_out_of_memory_error_java_heap,
                                     msg());

    msg = java_lang_String::create_from_str("Metaspace", CHECK_false);
    java_lang_Throwable::set_message(Universe::_out_of_memory_error_metaspace,
                                     msg());
    msg = java_lang_String::create_from_str("Compressed class space",
                                            CHECK_false);
    java_lang_Throwable::set_message(
        Universe::_out_of_memory_error_class_metaspace, msg());

    msg = java_lang_String::create_from_str(
        "Requested array size exceeds VM limit", CHECK_false);
    java_lang_Throwable::set_message(Universe::_out_of_memory_error_array_size,
                                     msg());

    msg = java_lang_String::create_from_str("GC overhead limit exceeded",
                                            CHECK_false);
    java_lang_Throwable::set_message(
        Universe::_out_of_memory_error_gc_overhead_limit, msg());

    msg = java_lang_String::create_from_str(
        "Java heap space: failed reallocation of scalar replaced objects",
        CHECK_false);
    java_lang_Throwable::set_message(
        Universe::_out_of_memory_error_realloc_objects, msg());

    msg = java_lang_String::create_from_str("/ by zero", CHECK_false);
    java_lang_Throwable::set_message(Universe::_arithmetic_exception_instance,
                                     msg());

    // Setup the array of errors that have preallocated backtrace
    k = Universe::_out_of_memory_error_java_heap->klass();
    assert(k->name() == vmSymbols::java_lang_OutOfMemoryError(),
           "should be out of memory error");
    k_h = instanceKlassHandle(THREAD, k);

    int len =
        (StackTraceInThrowable) ? (int)PreallocatedOutOfMemoryErrorCount : 0;
    Universe::_preallocated_out_of_memory_error_array =
        oopFactory::new_objArray(k_h(), len, CHECK_false);
    for (int i = 0; i < len; i++) {
  ...
  InstanceKlass::cast(SystemDictionary::Finalizer_klass())
      ->link_class(CHECK_false);
  Method *m = InstanceKlass::cast(SystemDictionary::Finalizer_klass())
                  ->find_method(vmSymbols::register_method_name(),
                                vmSymbols::register_method_signature());
  if (m == NULL || !m->is_static()) {
    tty->print_cr("Unable to link/verify Finalizer.register method");
    return false; // initialization failed (cannot throw exception yet)
  }
  Universe::_finalizer_register_cache->init(SystemDictionary::Finalizer_klass(),
                                            m);

  InstanceKlass::cast(SystemDictionary::misc_Unsafe_klass())
      ->link_class(CHECK_false);
  m = InstanceKlass::cast(SystemDictionary::misc_Unsafe_klass())
          ->find_method(vmSymbols::throwIllegalAccessError_name(),
  ...
  InstanceKlass::cast(SystemDictionary::ClassLoader_klass())
      ->link_class(CHECK_false);
  m = InstanceKlass::cast(SystemDictionary::ClassLoader_klass())
          ->find_method(vmSymbols::addClass_name(),
                        vmSymbols::class_void_signature());
  if (m == NULL || m->is_static()) {
    tty->print_cr("Unable to link/verify ClassLoader.addClass method");
    return false; // initialization failed (cannot throw exception yet)
  }
  Universe::_loader_addClass_cache->init(SystemDictionary::ClassLoader_klass(),
                                         m);

  // Setup method for checking protection domain
  InstanceKlass::cast(SystemDictionary::ProtectionDomain_klass())
      ->link_class(CHECK_false);
  ...
  if (m != NULL) {
    if (m->is_static()) {
      ...
      tty->print_cr("ProtectionDomain.impliesCreateAccessControlContext() has "
                    "the wrong linkage");
      return false; // initialization failed
    }
...
void Universe::flush_dependents_on(instanceKlassHandle dependee) {
  ...
  if (CodeCache::number_of_nmethods_with_dependencies() == 0)
    return;
...
void Universe::flush_evol_dependents_on(instanceKlassHandle ev_k_h) {
  ...
  if (CodeCache::mark_for_evol_deoptimization(ev_k_h) > 0) {
    ...
    // All this already happens inside a VM_Operation, so we'll do all the work
    // here. Stuff copied from VM_Deoptimize and modified slightly.

    // We do not want any GCs to happen while we are in the middle of this VM
    // operation
    ResourceMark rm;
    ...
    // Make the dependent methods not entrant (in VM_Deoptimize they are made
    // zombies)
...
void Universe::initialize_verify_flags() {
  ...
  while (token != NULL) {
    if (strcmp(token, "threads") == 0) {
      ...
      vm_exit_during_initialization(
          err_msg("VerifySubSet: \'%s\' memory sub-system is unknown, please "
                  "correct it",
                  token));
    }
...
void Universe::verify(VerifyOption option, const char *prefix, bool silent) {
  ...
  COMPILER2_PRESENT(assert(!DerivedPointerTable::is_active(),
  ...
  if (!silent)
    gclog_or_tty->print("%s", prefix);
  if (!silent)
    gclog_or_tty->print("[Verifying ");
  if (should_verify_subset(Verify_Threads)) {
    if (!silent)
      gclog_or_tty->print("Threads ");
    Threads::verify();
  }
  if (should_verify_subset(Verify_Heap)) {
    if (!silent)
      gclog_or_tty->print("Heap ");
    heap()->verify(silent, option);
  }
  if (should_verify_subset(Verify_SymbolTable)) {
    if (!silent)
      gclog_or_tty->print("SymbolTable ");
    SymbolTable::verify();
  }
  if (should_verify_subset(Verify_StringTable)) {
    if (!silent)
      gclog_or_tty->print("StringTable ");
    StringTable::verify();
  }
...
uintptr_t Universe::verify_oop_mask() {
  ...
  calculate_verify_data(_verify_oop_data, m.start(), m.end());
