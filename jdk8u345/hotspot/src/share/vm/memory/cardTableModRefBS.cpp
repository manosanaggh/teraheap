...
#ifdef TERA_CARDS
size_t CardTableModRefBS::th_cards_required(size_t covered_words) {
  // Add one for a guard card, used to detect errors.
  const size_t words = align_size_up(covered_words, th_card_size_in_words);
  return words / th_card_size_in_words + 1;
}

size_t CardTableModRefBS::th_compute_byte_map_size() {
  ...
  const size_t granularity = os::vm_allocation_granularity();
  return align_size_up(_th_guard_index + 1, MAX2(_th_page_size, granularity));
}

CardTableModRefBS::CardTableModRefBS(MemRegion whole_heap,
                                     int max_covered_regions,
                                     MemRegion th_whole_heap): 
  ...
  _th_byte_map_size(0),
  _th_byte_map(NULL),
  th_byte_map_base(NULL),
  // LNC functionality Teraheap
  _th_lowest_non_clean(NULL),
  _th_lowest_non_clean_chunk_size(0),
  _th_lowest_non_clean_base_chunk_index(NULL),
  _th_last_LNC_resizing_collection(NULL) {
  ...
  assert(th_card_size <= (1 << TERA_CARD_SIZE),
         err_msg("TeraCache card_size  must be less equall %d", (1 << TERA_CARD_SIZE)));

  _covered = new MemRegion[_max_covered_regions];
  if (_covered == NULL) {
    vm_exit_during_initialization(
        "Could not allocate card table covered region set.");
  }
}
  
void CardTableModRefBS::th_card_table_initialize() {
  ...
  const size_t th_rs_align =
      _th_page_size == (size_t)os::vm_page_size()
          ? 0
          : MAX2(_th_page_size, (size_t)os::vm_allocation_granularity());

  assert(_th_byte_map_size > 0, err_msg("_th_byte_map_size should not be zero"));
  ReservedSpace th_heap_rs(_th_byte_map_size, th_rs_align, false);
  ...
  _th_byte_map = (jbyte *)th_heap_rs.base();
  th_byte_map_base = _th_byte_map - (uintptr_t(th_low_bound) >> th_card_shift);
  
  DEBUG_ONLY(fprintf(stderr, "===================================\n"));
  DEBUG_ONLY(fprintf(stderr, "TeraHeap\n"));
  DEBUG_ONLY(fprintf(stderr, "CT_Base = %p \n", th_heap_rs.base()));
  DEBUG_ONLY(fprintf(stderr, "CT_End = %p \n", th_heap_rs.end()));
  DEBUG_ONLY(fprintf(stderr, "CT_ByteMapBase = %p \n", th_byte_map_base));
  DEBUG_ONLY(fprintf(stderr, "===================================\n"));
  ...
  jbyte *th_guard_card = &_th_byte_map[_th_guard_index];

  uintptr_t th_guard_page =
      align_size_down((uintptr_t)th_guard_card, _th_page_size);

  _th_guard_region = MemRegion((HeapWord *)th_guard_page, _th_page_size);
  ...
  *th_guard_card = last_card;
  ...
  _th_lowest_non_clean = NEW_C_HEAP_ARRAY(CardArr, _max_covered_regions, mtGC);
  _th_lowest_non_clean_chunk_size =
      NEW_C_HEAP_ARRAY(size_t, _max_covered_regions, mtGC);
  _th_lowest_non_clean_base_chunk_index =
      NEW_C_HEAP_ARRAY(uintptr_t, _max_covered_regions, mtGC);
  _th_last_LNC_resizing_collection =
      NEW_C_HEAP_ARRAY(int, _max_covered_regions, mtGC);
  if (_th_lowest_non_clean == NULL || _th_lowest_non_clean_chunk_size == NULL ||
      _th_lowest_non_clean_base_chunk_index == NULL ||
      _th_last_LNC_resizing_collection == NULL)
    vm_exit_during_initialization("couldn't allocate an LNC array.");

  for (int i = 0; i < _max_covered_regions; i++) {
    _th_lowest_non_clean[i] = NULL;
    _th_lowest_non_clean_chunk_size[i] = 0;
    _th_last_LNC_resizing_collection[i] = -1;
  }
}
...
#endif
{
  ...
  void CardTableModRefBS::initialize() {
    ...
    if (_committed == NULL) {
      vm_exit_during_initialization(
          "Could not allocate card table committed region set.");
    }

    const size_t rs_align =
        _page_size == (size_t)os::vm_page_size()
            ? 0
    ...
    assert(byte_for(high_bound - 1) <= &_byte_map[_last_valid_index],
           "Checking end of map");
  
  DEBUG_ONLY(fprintf(stderr, "===================================\n"));
  DEBUG_ONLY(fprintf(stderr, "Java Heap\n"));
  DEBUG_ONLY(fprintf(stderr, "CT_Base = %p \n", heap_rs.base()));
  DEBUG_ONLY(fprintf(stderr, "CT_End = %p \n", heap_rs.end()));
  DEBUG_ONLY(fprintf(stderr, "CT_ByteMapBase = %p \n", byte_map_base));
  DEBUG_ONLY(fprintf(stderr, "===================================\n"));
    ...
    *guard_card = last_card;
    ...
    _last_LNC_resizing_collection =
        NEW_C_HEAP_ARRAY(int, _max_covered_regions, mtGC);
    if (_lowest_non_clean == NULL || _lowest_non_clean_chunk_size == NULL ||
        _lowest_non_clean_base_chunk_index == NULL ||
        _last_LNC_resizing_collection == NULL)
      vm_exit_during_initialization("couldn't allocate an LNC array.");
    for (int i = 0; i < _max_covered_regions; i++) {
...
#ifdef TERA_CARDS
  if (EnableTeraHeap) {
    if (_th_lowest_non_clean) {
      FREE_C_HEAP_ARRAY(CardArr, _th_lowest_non_clean, mtGC);
      _th_lowest_non_clean = NULL;
    }
    if (_th_lowest_non_clean_chunk_size) {
      FREE_C_HEAP_ARRAY(size_t, _th_lowest_non_clean_chunk_size, mtGC);
      _th_lowest_non_clean_chunk_size = NULL;
    }
    if (_th_lowest_non_clean_base_chunk_index) {
      FREE_C_HEAP_ARRAY(uintptr_t, _th_lowest_non_clean_base_chunk_index, mtGC);
      _th_lowest_non_clean_base_chunk_index = NULL;
    }
    if (_th_last_LNC_resizing_collection) {
      FREE_C_HEAP_ARRAY(int, _th_last_LNC_resizing_collection, mtGC);
      _th_last_LNC_resizing_collection = NULL;
    }
  }
#endif
  ...
  MemRegion CardTableModRefBS::committed_unique_to_self(int self, MemRegion mr)
      const {
    MemRegion result = mr;
  ...
  void CardTableModRefBS::resize_covered_region(MemRegion new_region) {
    ...
    debug_only(verify_guard();)
        // collided is true if the expansion would push into another committed
        // region
        debug_only(bool collided = false;) int const ind =
            find_covering_region_by_base(new_region.start());
    ...
    if (new_region.word_size() != old_region.word_size()) {
      ...
      assert(new_end_aligned >= (HeapWord *)new_end, "align up, but less");
      ...
      if (new_end_for_commit > cur_committed.end()) {
        ...
        // Must uncommit pages.
        MemRegion const uncommit_region = committed_unique_to_self(
            ind, MemRegion(new_end_aligned, cur_committed.end()));
    ...
    if (TraceCardTableModRefBS) {
      ...
      gclog_or_tty->print_cr("  "
                             ...
                             ind, p2i(_covered[ind].start()), ind,
                             p2i(_covered[ind].last()));
      gclog_or_tty->print_cr("  "
                             "  _committed[%d].start(): " INTPTR_FORMAT
                             "  _committed[%d].last(): " INTPTR_FORMAT,
                             ind, p2i(_committed[ind].start()), ind,
                             p2i(_committed[ind].last()));
      gclog_or_tty->print_cr("  "
  ...
  void CardTableModRefBS::non_clean_card_iterate_possibly_parallel(
      Space * sp, MemRegion mr, OopsInGenClosure * cl, CardTableRS * ct) {
    if (!mr.is_empty()) {
      ...
      fatal("Parallel gc not supported here.");
        ...
        // We do not call the non_clean_card_iterate_serial() version below
        // because we want to clear the cards (which
        // non_clean_card_iterate_serial() does not do for us): clear_cl here
        // does the work of finding contiguous dirty ranges of cards to process
        // and clear.

        DirtyCardToOopClosure *dcto_cl =
            sp->new_dcto_cl(cl, precision(), cl->gen_boundary());
  ...
  void CardTableModRefBS::dirty_MemRegion(MemRegion mr) {
    assert((HeapWord *)align_size_down((uintptr_t)mr.start(), HeapWordSize) ==
               mr.start(),
           "Unaligned start");
    assert((HeapWord *)align_size_up((uintptr_t)mr.end(), HeapWordSize) ==
               mr.end(),
           "Unaligned end");
...
#ifdef TERA_CARDS
void CardTableModRefBS::th_write_ref_field(void *obj) {
  jbyte* card  = byte_for(obj);
  *card = dirty_card;
}

void CardTableModRefBS::th_clean_cards(HeapWord *start, HeapWord* end) {
...
void CardTableModRefBS::th_num_dirty_cards(HeapWord *start, HeapWord* end, bool before) {
    ...
    for (int i = 0; i < _cur_covered_regions; i++) {
      ...
      if (!mri.is_empty()) {
        ...
        for (cur_entry = byte_for(mri.start()), limit = byte_for(mri.last());
             cur_entry <= limit; cur_entry = next_entry) {
  ...
  MemRegion CardTableModRefBS::dirty_card_range_after_reset(
      MemRegion mr, bool reset, int reset_val) {
    for (int i = 0; i < _cur_covered_regions; i++) {
      MemRegion mri = mr.intersection(_covered[i]);
      if (!mri.is_empty()) {
        jbyte *cur_entry, *next_entry, *limit;
...
#ifdef TERA_CARDS
uint64_t CardTableModRefBS::th_ct_max_alignment_constraint() {
  ...
  void CardTableModRefBS::verify() { verify_guard(); }

#ifndef PRODUCT
  void CardTableModRefBS::verify_region(MemRegion mr, jbyte val,
                                        bool val_equals) {
    ...
    for (jbyte *curr = start; curr <= end; ++curr) {
      ...
      if (failed) {
        if (!failures) {
          tty->cr();
          tty->print_cr("== CT verification failed: [" INTPTR_FORMAT
                        "," INTPTR_FORMAT "]",
                        p2i(start), p2i(end));
  ...
  void CardTableModRefBS::print_on(outputStream * st) const {
    st->print_cr("Card table byte_map: [" INTPTR_FORMAT "," INTPTR_FORMAT
                 "] byte_map_base: " INTPTR_FORMAT,
                 p2i(_byte_map), p2i(_byte_map + _byte_map_size),
                 p2i(byte_map_base));
  }

  bool CardTableModRefBSForCTRS::card_will_be_scanned(jbyte cv) {
    return CardTableModRefBS::card_will_be_scanned(cv) ||
           _rs->is_prev_nonclean_card_val(cv);
  };

  bool CardTableModRefBSForCTRS::card_may_have_been_dirty(jbyte cv) {
    return cv != clean_card &&
