	  ...
	  while (map < end_map) {                                                 \
		  if (find_transient_field_from_offset(map->offset(), &fd)) {         \
			  InstanceKlass_SPECIALIZED_OOP_ITERATE(oop,                      \
					  obj->obj_field_addr<oop>(map->offset()), map->count(),  \
					  MarkSweep::mark_and_push(p), assert_fn)                 \
			  Universe::teraCache()->tc_mk_dirty(obj);                        \
		  }																      \
		  else {														      \
			  InstanceKlass_SPECIALIZED_OOP_ITERATE(oop,                      \
					  obj->obj_field_addr<oop>(map->offset()), map->count(),  \
...
void InstanceKlass::oop_follow_contents(oop obj) {
	...
	{
		Universe::teraCache()->set_cur_obj_group_id(obj->get_obj_group_id());
		Universe::teraCache()->set_cur_obj_num_access(obj->get_obj_num_accesses());
#if P_SD
...
void InstanceKlass::tc_oop_trace_contents(PSPromotionManager* pm, oop obj) {
...
const char* InstanceKlass::signature_name() const {
  ...
  if (is_anonymous()) {
    assertf(EnableInvokeDynamic, "EnableInvokeDynamic was not set.");
